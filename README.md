# Hi there, I'm [stephen mwangi] 👋

I am a Statistics and Economics student specializing in Data Science and Analytical Project Management. I love turning raw data into actionable insights through forecasting models, data cleaning, and programming.

## 🛠️ Tech Stack & Tools
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)
![Pandas](https://img.shields.io/badge/pandas-%23150458.svg?style=for-the-badge&logo=pandas&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

## 📈 Featured Projects
* **Macroeconomic & Credit Analysis** - Simulating and analyzing financial indicators using Python.
* **48-Month Sales Forecasting Model** - Linear regression and trend analysis built in Excel.
* **Project Monitoring Dashboard** - Interactive tracking system with automated status tools.

📫 **How to reach me:** [https://www.linkedin.com/posts/stephen-mwangi-a514a536a_excel-vba-dataanalytics-activity-7469053337053696002-ZqCK?utm_source=share&utm_medium=member_desktop&rcm=ACoAAFuNoZ0BzpA3eTLSNoBuud18PFbOqVqGfGM]
